...
def G_LSF(u, u2x, g_fun, model, x, y):
    return g_fun(np.array(u2x(u),ndmin=2), model, x, y)

def SuS(N, p0, g_fun, distr, sensitivity_analysis, samples_return, model, x, y):
    ...
    # G_LSF = lambda u: g_fun(np.array(u2x(u),ndmin=2), model, x, y)
    ...
    geval = G_LSF(u_j, u2x, g_fun, model, x, y).ravel()
    # print(geval)
    # print(geval.shape)
    ...
    while True:
        ...
        # select seeds
        ord_seeds = u_j_sort[:int(nF[j]),:]  # ordered level seeds
        ...
        [u_j, geval, lam, sigma, accrate] = aCS(N, lam, b[j], rnd_seeds, G_LSF, model, x, y, u2x, g_fun)
