...
# def aCS_P(Nchain, u_jp1, u_j, G_LSF, geval, rho, acc, sigma, b, k):
...
def aCS(N, old_lam, b, u_j, G_LSF, model, x, y, u2x, g_fun):
    ...
    # 并行计算
    # numbers = list(range(1, Ns+1))
    # Parallel(n_jobs = 15)(delayed(aCS_P)(Nchain, u_jp1, u_j, G_LSF, geval, rho, acc, sigma, b, k) for k in numbers)

    for k in range(1, Ns+1):
        idx          = sum(Nchain[:k-1])     # beginning of each chain index
        #acc[idx]     = 1                    # seed acceptance
        u_jp1[idx,:] = u_j[k-1,:]            # pick a seed at random
        geval[idx]   = G_LSF(u_jp1[idx,:], u2x, g_fun, model, x, y)   # store the lsf evaluation
        # print('k = %d'%k)
        ...
        for t in range(1, Nchain[k-1]):
            ...
            # accept or reject sample
            Ge = G_LSF(v, u2x, g_fun, model, x, y)
            # print('t = %d'%t)
            if Ge <= b:
                u_jp1[idx+t,:] = v         # accept the candidate in failure region
                geval[idx+t]   = Ge        # store the lsf evaluation
